import { PaginaParticipante } from './../model/PaginaParticipante.model';
...
export class ParticipanteService {
  ...
  update(participante: Participante): Observable<Participante> {
    return this.http.put<Participante>(`${this.apiURL}/${participante.id}`, participante);
  ...
  readTable(): Observable<ParticipanteResumoDTO[]> {
    return this.http.get<ParticipanteResumoDTO[]>(this.apiURL)
    /*.pipe(
      map((obj) => obj),
      catchError(e => this.errorHandler(e))
    ); */
  ...
  readTablePageable(page, size): Observable<PaginaParticipante> {
